...
public:
    ...
    double calculatePID(double target, double current)
    {
        ...
        if (pidCalc_scaled > 12000)
            pidCalc_scaled = 12000;
        if (pidCalc_scaled < -12000)
            pidCalc_scaled = -12000;
...
void umbc::Robot::opcontrol()
{
    ...
    while (1)
    {
        ...
        { // drives intake when timmer is active (only works here for some reason DON'T MOVE)
        ...
        { // locks motors into last recorded position to shoot ball at the end
